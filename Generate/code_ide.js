    ...
    setTimeout(() => {
      ...
      ${clonedDocument.documentElement.innerHTML}
            ...
            function extract_code_with_body(generatedContent) {
                ...
                const excludedScriptsStylesHTML = htmlCode.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>|<style\b[^<]*(?:(?!<\/style>)<[^<]*)*<\/style>/gi, '');
                ...
                codeTextarea_js.textContent = scriptCode;
            }
        ...
        <html lang="fr">
          ...
          ${clonedDocument.documentElement.innerHTML}
            ...
            const regex = /window\.addEventListener\([\s\S]*?window\.location\.replace\("\.\.\/index\.html"\);/;
